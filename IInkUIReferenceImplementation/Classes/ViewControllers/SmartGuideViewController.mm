...
- (void)smartGuideWordViewGetText:(SmartGuideWordView *)smartGuideWordView;
...
- (nullable instancetype)initWithWord:(SmartGuideWord *)word index:(NSUInteger)index isWhitespace:(BOOL)isWhitespace
{
 ...
 [self.delegate smartGuideWordViewGetText:self];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"notifyme" object: self.text];
...
- (void)updateWithBlock:(IINKContentBlock *)block cause:(UpdateCause)cause
{
    ...
    else
    {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:@"eraseallletters" object: nil];
        self.block = nil;
    }
